...
- Release-сборки macOS на `master` теперь требуют Apple Developer ID/notarization secrets, импортируют `Developer ID Application` certificate и передают signing/notarization env в `tauri-action`.
